...
plot(swo, swh, type = "n", xlim = c(-12,0), ylim = c(-80, 0), xlab = expression(paste(delta^{18}, "O (\u2030)")), ylab = expression(paste(delta^{2}, "H (\u2030)")))
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey")
abline(10, 8)
...
sum(results$Sprob)/nrow(results)
...
#set up space
jpeg("C:/Users/gjbowen/Dropbox/HypoMirror/WaterOrigin/Figure2.jpg", units = "cm", res = 600, width = 15, height = 8)
...
plot(results$O_h[1],results$H_h[1],xlim=c(lim[1], lim[2]), ylim=c(lim[3], lim[4]), 
     xlab=expression(paste(delta^{18}, "O (\u2030)")),
     ylab=expression(paste(delta^{2}, "H (\u2030)")))
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey")
abline(10, 8, lw=2)
lcols = ( 0.9 - 0.9*(results$Sprob[1:nex] * results$obs_prob[1:nex] * results$hypo_prob[1:nex])/max(results$Sprob[1:nex]* results$obs_prob[1:nex] * results$hypo_prob[1:nex]))
...
#Panel 2 - sensitivity test slope parameter
niter  = 50
...
plot(hsource_o, ppost, type="l", xlab=expression(paste("Hypothesized source ", delta^{18}, "O (\u2030)")), ylab=("Conditional probability"))
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey")
lines(hsource_o, ppost, lw=2, col="blue")
...
lines(hsource_o, ppost, lw=2, col="black")
...
lines(hsource_o, ppost, lw=2, col="red")
text(par("usr")[1] + (par("usr")[2]-par("usr")[1]) * 0.05,  par("usr")[4] - (par("usr")[4]-par("usr")[3]) * 0.1, "B")

hslope = c(4.5, 0.5)  #reset slope parameters

#Panel 3 - sensitivity test source uncertainty
niter  = 50
hsource_o = seq(-9, -5, 4/(niter-1))
ppost = vector(,niter)
...
lines(hsource_o, ppost, lw=2, col="red")
text(par("usr")[1] + (par("usr")[2]-par("usr")[1]) * 0.05,  par("usr")[4] - (par("usr")[4]-par("usr")[3]) * 0.1, "C")
...
#set up plot space
jpeg("C:/Users/gjbowen/Dropbox/HypoMirror/WaterOrigin/Figure3.jpg", units = "cm", res = 600, width = 7, height = 7)
par(mai=c(0.5,0.5,0.1,0.1), cex=0.5)
...
#now plot densities
jpeg("C:/Users/gjbowen/Dropbox/HypoMirror/WaterOrigin/Figure4.jpg", units = "cm", res = 600, width = 7, height = 6)
par(mai=c(0.5,0.5,0.1,0.1), cex=0.5)
plot(density(results$X1, from=0, to=1), main="", ylim=c(0,4), xlab="Source 1 fraction", type="n")
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey", border = "black")
lines(density(prior[,1], from=0, to=1), col="white", lw=2, lty=2)
lines(density(results$X1, from=0, to=1), col="black", lw=2, lty=2)
...
#now plot densities
lines(density(prior[,1], from=0, to=1), col="white", lw=2)
lines(density(results$X1, from=0, to=1), col="black", lw=2)
...
#set up
jpeg("C:/Users/gjbowen/Dropbox/HypoMirror/WaterOrigin/Figure5.jpg", units = "cm", res = 600, width = 7, height = 7)
par(mai=c(0.5,0.5,0.1,0.1), cex=0.5)
...
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey", border = "black")
points(epa_data$d18O, epa_data$d2H, pch=19)
abline(10, 8)
points(epa_data$o_sum, epa_data$h_sum, pch=21, col="red")
points(epa_data$o_win, epa_data$h_win, pch=21, col="blue")
points(epa_data$o_ann, epa_data$h_ann, pch=21, col="dark green")
...
#setup
jpeg("Figure6.jpg", units = "cm", res = 600, width = 13, height = 14)
...
plot(precip, legend.only=TRUE, col = pal(20), axis.args=list(cex.axis=0.6, tcl=-0.2, mgp=c(0,0.4,0)), 
     legend.args=list(text=expression(paste(delta^{2}, "H (\u2030)")), side = 4, line=1.5, cex=0.8))
text(-2100000, 800000, "A")
...
plot(el, legend.only=TRUE, col = pal(20), axis.args=list(cex.axis=0.6, tcl=-0.2, mgp=c(0,0.4,0)), 
     legend.args=list(text="Lake EL slope", side = 4, line=1.5, cex=0.8))
text(-2100000, 800000, "B")
...
jpeg("Figure7.jpg", units = "cm", res = 600, width = 14, height = 7)
layout(matrix(c(1,2), 1, 2, byrow = TRUE))
par(mai=c(0.5,0.5,0.1,0.1), cex=0.5)

#panel 1 showing conditional probs vs lake d18O 
plot(epa_data$d18O, prob_mean, type="n", xlab=expression(paste("Lake ", delta^{18}, "O (\u2030)")), ylab="Conditional probability")
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey")
points(epa_data$d18O, prob_mean, pch=21, bg="white")
text(par("usr")[1] + (par("usr")[2]-par("usr")[1]) * 0.05,  par("usr")[4] - (par("usr")[4]-par("usr")[3]) * 0.05, "A")

#panel 1 shotwing conditional probs vs lake Dex
plot((epa_data$d2H - 8 * epa_data$d18O), prob_mean, type="n", xlab="Lake D-excess (\u2030)", ylab="Conditional probability")
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey")
...
start = proc.time()
prob = foreach(i=1:3, .combine=cbind) %dopar% {
...
#set up map plot
jpeg("Figure8.jpg", units = "cm", res = 600, width = 13, height = 14)
layout(matrix(c(1,2), 2, 1, byrow = TRUE))
par(mai=c(0.1,0.1,0.1,0.1))
...
text(-2100000, 800000, "B")
legend(2200000, -1000000, c("> 0.3", "0.1 - 0.3", "-0.1 - 0.1", "-0.3 - -0.1"), bty="n", pch=21, pt.bg = pal(5)[5:2], cex=0.75)
...
#set up final plot
jpeg("Figure9.jpg", units = "cm", res = 600, width = 7, height = 7)
par(mai=c(0.5,0.5,0.1,0.1), cex=0.5)

#plot  
plot(res_summary$wfrac, 1 - res_summary$mx_prob_mean, type="n", xlab="Prior mean winter runoff fraction", ylab="Posterior mean winter runoff fraction")
rect(par("usr")[1], par("usr")[3], par("usr")[2], par("usr")[4], col = "grey")
abline(0,1)
